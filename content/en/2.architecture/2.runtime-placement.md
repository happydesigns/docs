...
---
Runtime placement decides where a business action is implemented. The same action should be written once and exposed through UI, API, jobs, webhooks, and MCP.

Use this page when a change could be written in several places. The goal is to keep the rule in one place and expose it through the right channels.
...
The diagram shows how runtime channels converge. A runtime channel is an entry point such as UI, HTTP API, scheduled job, webhook, or MCP tool. Each channel calls the same application service instead of reimplementing the action. The service applies policy and audit rules, then delegates business rules to a domain package.
...
These scenarios show where the shared rule lives and which channels may expose it.
