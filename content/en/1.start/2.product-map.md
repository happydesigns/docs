...
Read this map from top to bottom. The two shaded groups are different layers: foundation products are reused by application products. Documentation products are listed in the table, but not drawn as dependencies because they explain the system rather than build it.

```mermaid
flowchart TD
  subgraph Foundations["Foundation products"]
    ...
    ID --> Brand["<strong>brand</strong><br/><small>happydesigns brand layer</small>"]
    ID --> ClientIdentity["<strong>client identity layers</strong>"]
  end

  subgraph Applications["Application products"]
    Sites["<strong>customer websites</strong>"]
